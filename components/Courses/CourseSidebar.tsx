...
export default function CourseSidebar({ selectedCategory, setSelectedCategory }: CourseSidebarProps) {
  return (
    <div className="w-full">
      ...
      <div className="hidden lg:flex flex-col bg-white border border-slate-200 rounded-2xl p-4 shadow-sm w-full sticky top-24 max-h-[calc(100vh-140px)]">
        <h3 className="text-slate-400 font-extrabold text-[10px] tracking-wider uppercase px-3 py-2 mb-2 border-b border-slate-100 shrink-0">
          Categories
        </h3>
        
        <div className="overflow-y-auto flex-grow space-y-1 pr-1 max-h-[calc(100vh-200px)] scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
          {courseCategories.map((category) => {
            const active = selectedCategory === category;
            return (
              <button
                key={category}
                onClick={() => setSelectedCategory(category)}
                className={`group flex items-center gap-3 w-full px-4 py-3.5 rounded-xl text-xs font-bold text-left tracking-wide transition-all duration-250 cursor-pointer shrink-0 ${
                  active
