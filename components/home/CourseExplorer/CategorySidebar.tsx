...
// SVG Icons for each category (using Navy and Gold colors)
...
// Subtitle Helper matching Ratnaparkkhi branding
const GetCategoryDesc = (category: string) => {
  switch (category) {
    ...
    case 'Arts & Humanities':
      return 'BA / MA / BSW';
    case 'Management':
      return 'MBA, BBA, EMBA, PGDBA';
    case 'Law':
      return 'LLM';
    case 'Health & Allied Science':
      return 'Public Health';
    case 'Yoga & Naturopathy':
      return 'B.Sc / MA / Diploma Yoga';
    case 'Journalism & Mass Communication':
      return 'BJMC / MJMC';
    case 'Library Science':
      return 'B.Lib / M.Lib';
    case 'Diploma Courses':
      return 'After 10th';
    case 'ITI Trades':
      return 'NCVT Approved Trades';
    case 'Skill Development':
      return 'Skill India Programs';
    case 'Apprenticeship Programs':
...
export default function CategorySidebar({
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex lg:hidden overflow-x-auto pb-3 gap-2 no-scrollbar scroll-smooth">
        {categories.map((category) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3.5 py-2 rounded-full text-xs font-bold whitespace-nowrap transition-all border shrink-0 cursor-pointer ${
                isActive
                  ? 'bg-[#0B1F3A] border-[#0B1F3A] text-white shadow-sm'
                  : 'bg-white border-slate-200 text-[#0B1F3A] hover:border-[#0B1F3A]'
              }`}
            >
              ...
              <div className="text-left leading-none">
                <span className="block text-[10px] font-extrabold">{category}</span>
              </div>
            </button>
          );
        })}
